...
class ProductImageScraper:
    ...
    def get_product_images_requests(self, url: str) -> list:
        ...
        try:
            ...
            # Ścieżki URL które zawierają PRAWDZIWE zdjęcia produktu na mlamp.pl:

            # /hpeciai/.../pol_ps_...  → główna galeria produktu (product_shot)

            # /hpeciai/.../pol_pm_...  → miniaturka produktu (product_main)

            # Odrzucamy:

            # /data/include/cms/...     → logo sklepu, banery, elementy CMS

            # /data/lang/pol/producers/ → logo producenta

            # /data/lang/pol/traits/    → ikonki cech produktu

            # /data/lang/pol/available/ → wykres dostępności

            # /pl/products/hpeciai/...pol_il_ → obrazy powiązanych produktów

            # /hpeciai/.../pol_il_...   → obrazy innych produktów (product_image innych)



            ALLOWED_PATTERNS = ['/pol_ps_', '/pol_pm_']

            BLOCKED_PATTERNS = [

                '/data/include/', '/data/lang/', '/pol_il_',

                '/pl/products/', 'logo', 'banner', 'icon',
            ...
            for img in soup.find_all('img'):
                ...
                # Musi zawierać jeden z dozwolonych wzorców

                if not any(p in full_url for p in ALLOWED_PATTERNS):

                    continue

                # Nie może zawierać żadnego z blokowanych wzorców

                if any(p in full_url for p in BLOCKED_PATTERNS):
            ...
            logger.info(f"✓ Znaleziono {len(images)} obrazów produktu (pol_ps/pol_pm)")

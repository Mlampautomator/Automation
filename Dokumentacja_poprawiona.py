...
class ProductImageScraper:
    ...
    def get_product_images(self, soup, base_url: str) -> list:
        """
        Akceptuje tylko /pol_ps_ i /pol_pm_, pomija /pl/products/.
        """
        images = []
        for img in soup.find_all('img'):
            src = img.get('src') or img.get('data-src') or img.get('data-original')
            if not src or (not src.startswith('http') and not src.startswith('/')):
                continue
...
def read_urls_from_excel(filepath: str) -> list:
    try:
        ...
        urls = [
            ...
            if row[0] and isinstance(row[0], str) and row[0].strip().startswith('http')
        ]
...
def main():
    print("\n" + "=" * 60)
    print("  POBIERACZ OBRAZÓW PRODUKTÓW - MLAMP.PL  v4 (szybki)")
    print("=" * 60 + "\n")
    ...
    logger.info(f"Do przetworzenia: {len(urls)} produktów (równolegle: {MAX_WORKERS_PRODUCTS})\n")
    ...
    try:
        # Przetwarzamy wiele produktów równocześnie
        with ThreadPoolExecutor(max_workers=MAX_WORKERS_PRODUCTS) as executor:
            futures = {executor.submit(scraper.get_product_info, url): (i, url)
            ...
            for future in as_completed(futures):
                ...
                logger.info(f"[{done}/{len(urls)}] gotowe\n")
    except KeyboardInterrupt:
        logger.info("\nPrzerwano przez użytkownika")
 
    # Zachowaj oryginalną kolejność URL-i
    results = [results_map.get(i) for i in range(len(urls))]
    elapsed = time.time() - start
 
    logger.info(f"\n✓ Przetworzono: {sum(1 for r in results if r)}/{len(urls)} produktów")
